...
############################################################   Similarity ratio   #######################################################################
#Takes about 5-10 min to run with a target set of just 20. Maybe it can be optimized somehow?

def Simmol2VecFeaturization(smiles):#Needs packages that are incompatible I think.
    featurizer = dc.feat.Mol2VecFingerprint()
    features = featurizer(smiles)
    print(f'Number of featurized items: {len(features)} \nValues:')
    print(f'Length of features: {len(features)}')

...
class PropertiesTable(models.Model):
    pkey = models.AutoField(primary_key=True)
    drug = models.OneToOneField(max_length=20, unique=True, to=MainTable, related_name='eprops', on_delete=models.CASCADE)
...
class CalcPropertiesTable(models.Model):
    pkey = models.AutoField(primary_key=True)
    drug = models.OneToOneField(max_length=20, unique=True, to=MainTable, related_name='cprops', on_delete=models.CASCADE)
    properties = models.TextField(blank=True, null=True)  # This field type is a guess.
